...
export default function CallerXApp() {
  const [step, setStep] = useState<"email" | "phone_verify" | "permissions" | "app">("email");
  ...
  const sendMessage = (txt: string) => {
    ...
    setMessages((prev) => ({ ...prev, [activeChat.id]: [...(prev[activeChat.id] || []), m] }));
    setInputVal("");
  };
    ...
    <div className="flex flex-col h-screen w-full bg-white text-slate-800 select-none max-w-md mx-auto">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-xs p-5 shadow-2xl text-slate-800 border border-slate-200">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-sm">Add International Number</h3>
            ...
            <form onSubmit={handleAddContact} className="space-y-3">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0088cc]" 
                />
              </div>
              <div>
                ...
                >
                  {COUNTRY_CODES.map((item) => (
                    <option key={item.code} value={item.code}>{item.flag} {item.name} ({item.code})</option>
                  ))}
                </select>
              </div>
              <div>
      ...
      <div className="p-3 bg-white flex items-center justify-between px-6 border-t border-slate-200">
        <button onClick={() => setShowAddModal(true)} className="text-[#0088cc] text-xl font-bold">
